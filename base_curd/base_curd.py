...
class BaseCURDView(RetrieveAPIView):
    # RetrieveAPIView 用来根据id(pk)取到对应的对象
    model = None
    ...
    def get_obj_by_id(self, request, *args, **kwargs):
        """
        根据id（pk）查询单个对象
        id 是从url路径中取值，根据 lookup_field 字段
        返回序列化好的数据
        """
        try:
    ...
    def delete_obj_by_ids(self, request, *args, **kwargs):
        try:
            ...
            if not isinstance(ids_data, list):
                raise BaseCustomException("入参ids必须是一个列表")
            if len(ids_data) == 0:
                raise BaseCustomException("入参ids为空")
    ...
    def soft_delete_obj_by_ids(self, request, *args, **kwargs):
        try:
            ...
            ids_data = request.data.get("ids", [])
            if not isinstance(ids_data, list):
                raise BaseCustomException(detail="入参ids必须是一个列表")
            if len(ids_data) == 0:
